...
// Note : S=4
// Polynomial order and trajectory dimension are fixed here
typedef Eigen::Matrix<double, 3, 8> CoefficientMat_S4;
typedef Eigen::Matrix<double, 3, 7> VelCoefficientMat_S4;
typedef Eigen::Matrix<double, 3, 6> AccCoefficientMat_S4;
class Piece_S4 {
 private:
  double duration;
 ...
 public:
  ...
  Piece_S4(double dur, const CoefficientMat_S4 &cMat)
      : duration(dur), coeffMat(cMat) {}
  ...
  inline const CoefficientMat_S4 &getCoeffMat() const {
    return coeffMat;
  }
  ...
  inline Eigen::Vector3d getPos(const double &t) const {
    ...
    double tn = 1.0;
    for (int i = 7; i >= 0; i--) {
      pos += tn * coeffMat.col(i); // 第一列是最高次数项对应的系数，最后一列是常数项
      tn *= t;
    }
    return pos;
  }
  ...
  inline CoefficientMat_S4 normalizePosCoeffMat() const {
    CoefficientMat_S4 nPosCoeffsMat;
    double t = 1.0;
    for (int i = 7; i >= 0; i--) {
      nPosCoeffsMat.col(i) = coeffMat.col(i) * t;
      t *= duration;
  ...
  inline VelCoefficientMat_S4 normalizeVelCoeffMat() const {
    VelCoefficientMat_S4 nVelCoeffMat;
    int n = 1;
    double t = duration;
    for (int i = 6; i >= 0; i--) {
      nVelCoeffMat.col(i) = n * coeffMat.col(i) * t;
  ...
  inline AccCoefficientMat_S4 normalizeAccCoeffMat() const {
    AccCoefficientMat_S4 nAccCoeffMat;
    int n = 2;
    int m = 1;
    double t = duration * duration;
    for (int i = 5; i >= 0; i--) {
  ...
  inline double getMaxVelRate() const {
    VelCoefficientMat_S4 nVelCoeffMat = normalizeVelCoeffMat();
    Eigen::VectorXd coeff = RootFinder::polySqr(nVelCoeffMat.row(0)) +
                            RootFinder::polySqr(nVelCoeffMat.row(1)) +
                            RootFinder::polySqr(nVelCoeffMat.row(2));
    int N = coeff.size();
  ...
  inline double getMaxAccRate() const {
    AccCoefficientMat_S4 nAccCoeffMat = normalizeAccCoeffMat();
    Eigen::VectorXd coeff = RootFinder::polySqr(nAccCoeffMat.row(0)) +
                            RootFinder::polySqr(nAccCoeffMat.row(1)) +
                            RootFinder::polySqr(nAccCoeffMat.row(2));
    int N = coeff.size();
...
class Trajectory_S4 {
 private:
  typedef std::vector<Piece_S4> Pieces;
  Pieces pieces;

 public:
  Trajectory_S4() = default;

  Trajectory_S4(const std::vector<double> &durs,
             const std::vector<CoefficientMat_S4> &cMats) {
    int N = std::min(durs.size(), cMats.size());
    pieces.reserve(N);
    for (int i = 0; i < N; i++) {
      pieces.emplace_back(durs[i], cMats[i]);
  ...
  inline Eigen::VectorXd getDurations() const {
    int N = getPieceNum(); // 段数
    Eigen::VectorXd durations(N);
    for (int i = 0; i < N; i++) {
      durations(i) = pieces[i].getDuration(); //每段的时间
    }
    return durations;
  }
  ...
  inline double getTotalDuration() const {
    ...
    double totalDuration = 0.0;
    for (int i = 0; i < N; i++) {
      totalDuration += pieces[i].getDuration(); // 全部加起来
    }
    return totalDuration;
  }
  ...
  inline const Piece_S4 &operator[](int i) const {
    return pieces[i];
  }

  inline Piece_S4 &operator[](int i) {
    return pieces[i];
  }
  ...
  inline void emplace_back(const Piece_S4 &piece) {
    pieces.emplace_back(piece);
    return;
  }

  inline void emplace_back(const double &dur,
                           const CoefficientMat_S4 &cMat) {
    pieces.emplace_back(dur, cMat);
    return;
  }

  inline void append(const Trajectory_S4 &traj) {
    pieces.insert(pieces.end(), traj.begin(), traj.end());
    return;
  }
