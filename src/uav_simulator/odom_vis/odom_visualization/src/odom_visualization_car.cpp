...
void fov_visual_init(std::string msg_frame_id) {
  ...
  fov_node.resize(5); //5个点
  fov_node[0][0] = 0;// 三个坐标
  fov_node[0][1] = 0;
  fov_node[0][2] = 0;

  fov_node[1][2] = x_max_dis; // fov四个角
  ...
  markerNode_fov.type = visualization_msgs::Marker::SPHERE_LIST; // 球序列
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  q(0) = msg->pose.pose.orientation.z;
  q(1) = 0;
  q(2) = 0;
  ...
  pub_fov_visual(fov_p, fov_q); // 可视化fov
