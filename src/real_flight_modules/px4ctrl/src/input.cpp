...
void RC_Data_t::sim_feed(sim_node::sim_rccmdConstPtr pMsg)
{
    sim_msg=*pMsg;
    rcv_stamp_sim=ros::Time::now();

    mode_sim  = pMsg->sim_rccmd_ch5;
    gear_sim  = pMsg->sim_rccmd_ch6;
    reboot_cmd_sim = pMsg->sim_rccmd_ch7;

        // 初始化
    if (!have_init_last_mode)
    {
        have_init_last_mode = true;
        last_mode_sim = mode_sim;
    }
    if (!have_init_last_gear)
    {
        have_init_last_gear = true;
        last_gear_sim = gear_sim;
    }
    if (!have_init_last_reboot_cmd)
    {
        have_init_last_reboot_cmd = true;
        last_reboot_cmd_sim = reboot_cmd_sim;
    }

    if (last_mode_sim ==sim_node::sim_rccmd::is_not_hover && mode_sim == sim_node::sim_rccmd::is_hover)
        {enter_hover_mode = true;
        ROS_INFO("enter hover mode!");
        }
    else
        enter_hover_mode = false;

    if (mode_sim == sim_node::sim_rccmd::is_hover)
        is_hover_mode = true; // 保持在px4
    else
        is_hover_mode = false;
    ...
    if (is_hover_mode)
    {
        if (last_gear_sim == sim_node::sim_rccmd::is_not_cmd && gear_sim == sim_node::sim_rccmd::is_cmd) //进入offboard模式
            {
                enter_command_mode = true;
                ROS_INFO("enter command mode!");
            }
        else if (gear < GEAR_SHIFT_VALUE)
            enter_command_mode = false;

        if (gear_sim == sim_node::sim_rccmd::is_cmd) // 保持offboard模式
            is_command_mode = true;
        else
            is_command_mode = false;
    }

    if (!is_hover_mode && !is_command_mode) // 同时为假
    {
        if (last_reboot_cmd_sim == sim_node::sim_rccmd::is_not_reboot && reboot_cmd_sim == sim_node::sim_rccmd::is_reboot)
            {
                toggle_reboot = true;
                ROS_INFO("toggle_reboot");
            }
...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    if (is_hover_mode)
    {
        if (last_gear < GEAR_SHIFT_VALUE && gear > GEAR_SHIFT_VALUE) //进入offboard模式
            enter_command_mode = true;
        else if (gear < GEAR_SHIFT_VALUE)
            enter_command_mode = false;

        if (gear > GEAR_SHIFT_VALUE) // 保持offboard模式
            is_command_mode = true;
        else
            is_command_mode = false;
    }
...
LaserGround_t::LaserGround_t()
{
    rcv_stamp = ros::Time(0);
}

// void LaserGround_t::feed(std_msgs::Int32MultiArrayConstPtr pMsg)
// {
//     rcv_stamp = ros::Time::now();

//     laser_ground_mm = pMsg->data[0];
//     amp = pMsg->data[1];
//     temp = pMsg->data[2];

//     // printf("px4ctrl  laser = %d mm\r\n", laser_ground_mm);
// }

void LaserGround_t::feed(nlink_parser::TofsenseFrame0ConstPtr pMsg)
{
    rcv_stamp = ros::Time::now();

    laser_ground_mm = (int)(pMsg->dis*1000);

    // using namespace std;
    // cout<<laser_ground_mm<<endl;
...
// 在仿真中获取laser信息
void LaserGround_t::sim_feed(sensor_msgs::LaserScanConstPtr pMsg)
{
    rcv_stamp = ros::Time::now();

    // laser_ground_mm = (int)(pMsg->dis*1000);
    laser_ground_mm = (int)(pMsg->ranges[0]*1000); //! 改了这里
