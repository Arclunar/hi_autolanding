...
public:
  ...
  uint8_t mode_sim;
  uint8_t gear_sim;
  uint8_t reboot_cmd_sim;
  uint8_t last_mode_sim;
  uint8_t last_gear_sim;
  uint8_t last_reboot_cmd_sim;
  ...
  sim_node::sim_rccmd sim_msg; // 添加msg成员变量
  ros::Time rcv_stamp;
  ros::Time rcv_stamp_sim;
  ...
  void sim_feed(sim_node::sim_rccmdConstPtr pMsg); // 添加仿真使用的feed
...
public:
  ...
  LaserGround_t();
  void feed(nlink_parser::TofsenseFrame0ConstPtr pMsg);
  void sim_feed(sensor_msgs::LaserScanConstPtr pMsg);
};
