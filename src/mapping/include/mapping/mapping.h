...
struct RingBuffer {
 public:
  ...
  inline const _Datatype& at(const Eigen::Vector3i& ad) const {
    return data[(ad.z() * size_y + ad.y()) * size_x + ad.x()]; // 返回数据
  }
  inline _Datatype& at(const Eigen::Vector3i& ad) {
    return data[(ad.z() * size_y + ad.y()) * size_x + ad.x()];
  }
  inline _Datatype* atPtr(const Eigen::Vector3i& ad) { // 直接引用
    return &(data[(ad.z() * size_y + ad.y()) * size_x + ad.x()]);
  }
...
struct OccGridMap : public RingBuffer<int8_t> {
 ...
 public:
  ...
  inline void setOcc(const Eigen::Vector3i& id) {
    if (isInMap(id)) {
 ...
 private:
  ...
  inline bool filter(const Eigen::Vector3d& sensor_p,
    ...
    double dist = dp.norm();
    pt = p;
    if (dist < sensor_range && isInMap(id)) { // 距离传感器距离距离小于range，在地图范围内
      return true;
 ...
 public:
  ...
  /// @param ld 左下角
  /// @param ru 右上角
  inline void setOccupied(const Eigen::Vector3d& ld,const Eigen::Vector3d & ru)
  {
      Eigen::Vector3i id_ld = pos2idx(ld);
      Eigen::Vector3i id_ru = pos2idx(ru);
      Eigen::Vector3i id;
